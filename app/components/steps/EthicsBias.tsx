...
export function EthicsBias() {
  ...
  const handleDownloadCertificate = () => {
    ...
    const checkPage = (needed = 12) => { if (y > 272 - needed) newPage(); };
    ...
    const row = (label: string, val: string, labelColor: [number,number,number] = [80,80,80], valColor: [number,number,number] = [20,20,20]) => {
      checkPage(7);
      doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(...labelColor);
    ...
    const divider = (light = false) => {
      doc.setDrawColor(light ? 220 : 160, light ? 220 : 160, light ? 220 : 160);
    ...
    doc.text("HealthML — Model Summary Certificate", margin, 10);
    doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(180, 210, 240);
    doc.text("Educational ML Visualisation Tool  ·  For Healthcare Professionals / Students", margin, 16);
    y = 28;

    doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(100, 100, 100);
    doc.text(`Generated: ${nowFull}`, margin, y); y += 5;
    doc.text(`Specialty explored: ${specialty || "Not specified"}`, margin, y); y += 5;
    doc.text(`Model trained: ${r?.modelType ?? "—"}`, margin, y); y += 3;
    divider();

    // ── SECTION 1: DATASET & TRAINING SETUP ────────────────────────────────
    sectionTitle("1  DATASET & TRAINING SETUP");
    row("Specialty", specialty || "Not specified");
    row("Model type", r?.modelType ?? "—");
    row("Training patients", `${r?.split?.trainCount ?? "—"}  (${r?.split?.trainPct ?? "—"}% of dataset)`);
    row("Test patients", `${r?.split?.testCount ?? "—"}  (${100 - (r?.split?.trainPct ?? 0)}% of dataset — never seen during training)`);
    row("Training latency", r?.trainingLatencyMs != null ? `${r.trainingLatencyMs} ms  (target < 3 000 ms)` : "—");
    if (r?.featureColumns?.length) {
      checkPage(8);
      doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(80,80,80);
      doc.text("Feature columns used:", margin, y); y += 5;
      doc.setFont("helvetica", "normal"); doc.setTextColor(40,40,40);
      const cols = (r.featureColumns as string[]).join("  ·  ");
      const colLines = doc.splitTextToSize(cols, contentW);
      colLines.forEach((l: string) => { checkPage(); doc.text(l, margin + 4, y); y += 4.5; });
      y += 2;
    }

    // ── SECTION 2: PERFORMANCE METRICS ──────────────────────────────────────
    sectionTitle("2  PERFORMANCE METRICS");
    const thresholds: Record<string, {green: number; amber: number}> = r?.thresholds ?? {};
    const metricRows: [string, number | undefined, string][] = [
      ["Accuracy", m?.accuracy, "% correctly classified of all test patients"],
      ["Sensitivity ★", m?.sensitivity, "% of truly positive patients caught (most critical for screening)"],
      ["Specificity", m?.specificity, "% of truly negative patients correctly called safe"],
      ["Precision", m?.precision, "% of flagged patients who actually had the condition"],
      ["F1 Score", m?.f1, "Balance between sensitivity and precision"],
      ["AUC-ROC", m?.auc, "Overall separation ability (0.5 = random, 1.0 = perfect)"],
    ];

    metricRows.forEach(([label, val, desc]) => {
      checkPage(9);
      const pct = val != null ? (label === "AUC-ROC" ? val.toFixed(2) : (val * 100).toFixed(1) + "%") : "—";
      const key = label.replace(" ★","").toLowerCase().replace("-roc","").replace(/ /g,"") as any;
      const t = thresholds[key === "aucroc" ? "auc" : key];
      let statusColor: [number,number,number] = [30,30,30];
      if (val != null && t) {
        statusColor = val >= t.green ? [22,101,52] : val >= t.amber ? [146,64,14] : [153,27,27];
      }
      ...
      doc.text(pct, margin + 52, y);
      doc.setFont("helvetica", "normal"); doc.setTextColor(110,110,110); doc.setFontSize(8);
      doc.text(desc, margin + 68, y);
      y += 6.5;
    });

    // ── SECTION 3: CONFUSION MATRIX ─────────────────────────────────────────
    y += 1;
    sectionTitle("3  CONFUSION MATRIX");
    const cx = margin, cw = (contentW - 4) / 2;
    checkPage(28);

    // Header row
    doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(255,255,255);
    doc.setFillColor(60,80,120); doc.rect(cx, y, cw, 7, "F");
    doc.setFillColor(60,80,120); doc.rect(cx + cw + 4, y, cw, 7, "F");
    doc.text("AI Predicted: NOT at Risk", cx + 2, y + 5);
    doc.text("AI Predicted: AT RISK", cx + cw + 6, y + 5);
    y += 7;

    const cmCell = (val: number | undefined, label: string, bg: [number,number,number], x: number, w: number) => {
      doc.setFillColor(...bg); doc.rect(x, y, w, 16, "F");
      doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.setTextColor(20,20,20);
      doc.text(String(val ?? "—"), x + w/2, y + 8, { align: "center" });
      doc.setFontSize(7.5); doc.setFont("helvetica", "normal"); doc.setTextColor(60,60,60);
      doc.text(label, x + w/2, y + 13, { align: "center" });
    };

    cmCell(cm?.tn, "True Negative — correctly safe", [220,252,231], cx, cw);
    cmCell(cm?.fp, "False Positive — unnecessary alarm", [254,243,199], cx + cw + 4, cw);
    y += 16;
    cmCell(cm?.fn, "False Negative — MISSED CASE ⚠", [254,226,226], cx, cw);
    cmCell(cm?.tp, "True Positive — correctly flagged", [220,252,231], cx + cw + 4, cw);
    y += 19;

    if (cm?.fn != null && cm.fn > 0) {
      text(`⚠  ${cm.fn} patient(s) with the condition were missed (False Negatives). In screening, these are the most dangerous errors.`, margin, 9, "bold", [153,27,27]);
      y += 2;
    }

    // ── SECTION 4: SUBGROUP FAIRNESS ────────────────────────────────────────
    sectionTitle("4  SUBGROUP FAIRNESS AUDIT");
    if (fairnessData?.subgroups.length) {
      // Table header
      checkPage(10);
      doc.setFillColor(240,244,248); doc.rect(margin, y-2, contentW, 7, "F");
      doc.setFontSize(8.5); doc.setFont("helvetica", "bold"); doc.setTextColor(50,50,80);
      doc.text("Patient Group", margin+1, y+3);
      doc.text("Accuracy", margin+45, y+3);
      doc.text("Sensitivity", margin+72, y+3);
      doc.text("Specificity", margin+99, y+3);
      doc.text("Fairness", margin+126, y+3);
      y += 8;
      fairnessData.subgroups.forEach((s, i) => {
        checkPage(8);
        if (i % 2 === 0) { doc.setFillColor(250,251,252); doc.rect(margin, y-3, contentW, 7, "F"); }
        const statusColor: [number,number,number] = s.status === "good" ? [22,101,52] : s.status === "warn" ? [146,64,14] : [153,27,27];
        doc.setFontSize(8.5); doc.setFont("helvetica", "normal"); doc.setTextColor(40,40,40);
        doc.text(s.group, margin+1, y+1);
        doc.text(s.accuracy, margin+45, y+1);
        doc.setFont("helvetica", "bold"); doc.setTextColor(...statusColor);
        doc.text(s.sensitivity, margin+72, y+1);
        doc.setFont("helvetica", "normal"); doc.setTextColor(40,40,40);
        doc.text(s.specificity, margin+99, y+1);
        doc.setTextColor(...statusColor);
        doc.text(s.fairness, margin+126, y+1);
        y += 7;
      });
      y += 2;
      if (fairnessData.biasDetected) {
        checkPage(10);
        doc.setFillColor(254,226,226); doc.rect(margin, y-1, contentW, 10, "F");
        doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(153,27,27);
        const biasLines = doc.splitTextToSize(`BIAS DETECTED: ${fairnessData.biasMessage}`, contentW - 4);
        biasLines.forEach((l: string) => { doc.text(l, margin+2, y+4); y += 4.5; });
        y += 4;
      } else {
        checkPage(8);
        doc.setFillColor(220,252,231); doc.rect(margin, y-1, contentW, 8, "F");
        doc.setFontSize(9); doc.setFont("helvetica", "bold"); doc.setTextColor(22,101,52);
        doc.text("No significant bias detected across measured subgroups.", margin+2, y+4);
        y += 10;
      }
    } else {
      text("No subgroup data available — no gender or age columns detected in the dataset.", margin, 9, "normal", [100,100,100]);
      y += 4;
    }

    // ── SECTION 5: EU AI ACT COMPLIANCE CHECKLIST ───────────────────────────
    sectionTitle("5  EU AI ACT COMPLIANCE CHECKLIST");
    const checked = checklist.filter(i => i.checked);
    const pending = checklist.filter(i => !i.checked);
    text(`${checked.length} of ${checklist.length} items completed`, margin, 9, "normal", [80,80,80]);
    y += 4;
    checklist.forEach((item) => {
      checkPage(10);
      const isChecked = item.checked;
      const bg: [number,number,number] = isChecked ? [220,252,231] : [254,242,242];
      doc.setFillColor(...bg); doc.rect(margin, y-3, contentW, 8, "F");
      doc.setFontSize(9.5);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(isChecked ? 22 : 153, isChecked ? 101 : 27, isChecked ? 52 : 27);
      doc.text(isChecked ? "[✓]" : "[ ]", margin+1, y+2);
      doc.setFont("helvetica", isChecked ? "bold" : "normal");
      doc.setTextColor(30,30,30);
      const itemLines = doc.splitTextToSize(item.text, contentW - 14);
      itemLines.forEach((l: string, li: number) => {
        if (li > 0) { checkPage(); }
        doc.text(l, margin+11, y+2+(li*4.5));
      });
      y += 4 + (itemLines.length - 1) * 4.5 + 3;
    });
    ...
    doc.text("This certificate was generated by HealthML — an educational tool for healthcare professionals and students.", margin, y); y += 4;
    doc.text("It is NOT a clinical report and must NOT be used for diagnosis, treatment, or patient care decisions.", margin, y); y += 4;
    doc.text("Always seek qualified medical oversight before acting on any AI prediction.", margin, y);

    doc.save(`HealthML_Certificate_${now}.pdf`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <button onClick={handleDownloadCertificate} className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition cursor-pointer text-[14px]">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
            ...
            <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl mt-4">
              ...
              <div className="text-[13px] text-amber-900">
                <span className="font-bold">Under-representation risk:</span> If training data does not match real patient demographics, the model will underperform for under-represented groups. Always retrain with representative data.
              </div>
            </div>
          </div>
