...
export function EthicsBias() {
  ...
  const handleDownloadCertificate = () => {
    ...
    const checkedItems = checklist.filter((i) => i.checked).map((i) => `  [x] ${i.text}`).join("\n");
    const uncheckedItems = checklist.filter((i) => !i.checked).map((i) => `  [ ] ${i.text}`).join("\n");
    const subgroupRows = fairnessData?.subgroups.map(
      (s) => `  ${s.group.padEnd(20)} Acc: ${s.accuracy}  Sens: ${s.sensitivity}  Spec: ${s.specificity}  ${s.fairness}`
    ).join("\n") || "  No subgroup data available.";

    const cert = [
      "=".repeat(60),
      "  HealthML — Model Summary Certificate",
      `  Generated: ${now}`,
      "=".repeat(60),
      "",
      "MODEL PERFORMANCE",
      ...
      `  Accuracy    : ${m ? (m.accuracy * 100).toFixed(1) + "%" : "—"}`,
      `  Sensitivity : ${m ? (m.sensitivity * 100).toFixed(1) + "%" : "—"}`,
      `  Specificity : ${m ? (m.specificity * 100).toFixed(1) + "%" : "—"}`,
      `  Precision   : ${m ? (m.precision * 100).toFixed(1) + "%" : "—"}`,
      `  F1 Score    : ${m ? (m.f1 * 100).toFixed(1) + "%" : "—"}`,
      `  AUC-ROC     : ${m ? (m.auc * 100).toFixed(1) + "%" : "—"}`,
      `  Train split : ${r?.split?.trainPct ?? "—"}% (${r?.split?.trainCount ?? "—"} patients)`,
      `  Test split  : ${100 - (r?.split?.trainPct ?? 0)}% (${r?.split?.testCount ?? "—"} patients)`,
      "",
      "SUBGROUP FAIRNESS",
      "-".repeat(40),
      subgroupRows,
      fairnessData?.biasDetected
        ? `\n  ⚠ BIAS DETECTED: ${fairnessData.biasMessage}`
        : "\n  No significant bias detected.",
      "",
      "EU AI ACT COMPLIANCE CHECKLIST",
      "-".repeat(40),
      checkedItems || "  (none checked)",
      uncheckedItems ? `\nPENDING:\n${uncheckedItems}` : "",
      "",
      "=".repeat(60),
      "  This certificate was generated by HealthML (educational tool).",
      "  Not for clinical use. Always seek qualified medical oversight.",
      "=".repeat(60),
    ].join("\n");
    ...
    a.href = url;
    a.download = `HealthML_Certificate_${now}.txt`;
    a.click();
